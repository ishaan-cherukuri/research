"""
Brain segmentation using SynthSeg for fast, robust volumetric analysis.
Alternative to FreeSurfer that runs in 1-2 minutes per scan.
"""
...
class BrainSegmenter:
    """Fast brain segmentation using SynthSeg or fallback methods."""
    ...
    def __init__(self, method: str = "synthseg", use_s3: bool = False):
        ...
        Args:
            method: 'synthseg', 'simple', or 'fsl'
            use_s3: Whether to support S3 file access
        """
        self.method = method
        self.use_s3 = use_s3
        if use_s3:
    ...
    def segment(
        ...
        """
        Segment brain and extract volumetric features.

        Args:
            nii_path: Path to input NIfTI file (local or S3)
            output_dir: Optional directory to save segmentation results
            is_s3: Whether the nii_path is an S3 location
    ...
    def _segment_synthseg(
        ...
        """
        Segment using SynthSeg (fast, accurate).

        SynthSeg provides volumes for:
        - Hippocampus (L/R)
        - Amygdala (L/R)
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                cmd = [
                    ...
                    "--qc",
                    str(qc_file),
                    "--robust",  # More robust to pathology and artifacts
                ]
    ...
    def _parse_synthseg_volumes(self, vol_file: Path) -> Dict:
        """Parse SynthSeg volume CSV output."""
        features = {}
        ...
        try:
            ...
            if len(lines) >= 2:
                ...
                # TIV and brain parenchymal fraction
                ...
                if all(v is not None for v in [gm, wm, csf]):
                    ...
                    if tiv > 0:
                        ...
                        # Normalized volumes (important for comparing across subjects)
    ...
    def _segment_simple(self, nii_path: str, is_s3: bool = False) -> Dict:
        """
        Simple intensity-based segmentation fallback.
        Fast but less accurate - provides rough tissue volumes.
        """
        features = {}
        ...
        try:
            ...
            # This is a crude approximation but fast
            brain_mask = data > (data.mean() * 0.1)
            brain_voxels = data[brain_mask]

            if len(brain_voxels) > 0:
                # Use intensity percentiles to separate tissues
                ...
                # Rough ventricle estimate (very low intensity in brain)
...
def test_segmentation():
    """Test segmentation on a sample file."""
    # Find a local nii file
    data_dir = Path("data")
    ...
    # Test simple method (always works)
