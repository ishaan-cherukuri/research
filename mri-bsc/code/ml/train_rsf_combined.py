...
def load_and_merge(slopes_path, survival_path, t1_path):
    ...
    available = [c for c in T1_FEATURE_COLS if c in t1_raw.columns]
    missing   = [c for c in T1_FEATURE_COLS if c not in t1_raw.columns]
    if missing:
        print(f"  WARNING: T1 columns not found (skipped): {missing}")
    t1 = t1_raw[["subject"] + available].copy()
...
def signed_log1p_df(X):
    return np.sign(X) * np.log1p(np.abs(X))

def fit_winsor_limits(X, lower_q, upper_q):
...
def select_slope_features_train_only(X_train, top_k=20, penalize_regex="nboundary",
                                     penalty_factor=0.10, winsor_q_low=0.01,
                                     winsor_q_high=0.99, quiet=False):
    ...
    score   = raw_var.copy()
    mask    = pd.Series(score.index.str.contains(penalize_regex, case=False, regex=True),
                        index=score.index)
    score.loc[mask] *= penalty_factor
...
def train_rsf_model(X_train, X_test, y_train, y_test, n_estimators=1000):
    y_tr = Surv.from_dataframe("event", "time_years", y_train)
    y_te = Surv.from_dataframe("event", "time_years", y_test)

    print(f"\nFitting RSF  (n_estimators={n_estimators}, "
          "min_split=10, min_leaf=15, max_features=sqrt) ...")
    ...
    train_c = rsf.score(X_train, y_tr)
    test_c  = rsf.score(X_test,  y_te)
    ...
    metrics = {
        ...
        "overfitting_gap": round(float(train_c - test_c), 4),
        "n_features":   X_train.shape[1],
        "n_estimators": n_estimators,
    }

    print(f"\n{'='*80}")
    print("RESULTS: RSF  (BSC slopes + T1 morphometry)")
    print(f"{'='*80}")
    ...
    return rsf, metrics, pred_train, pred_test

def run_5fold_cv(df, slope_cols, t1_cols, y_all, n_estimators,
                 top_k, penalize_regex, penalty_factor, winsor_low, winsor_high):
...
def plot_cv_boxplots(cv_results, out_path):
    fig, axes = plt.subplots(1, 3, figsize=(12, 6))
    fig.suptitle(
        "RSF  —  BSC Slopes + T1 Morphometry\n"
...
def create_km_curves(y_train, y_test, pred_train, pred_test, out_path):
    ...
    plt.tight_layout()
    plt.savefig(out_path, dpi=300, bbox_inches="tight")
    plt.savefig(Path(out_path).with_suffix(".pdf"), bbox_inches="tight")
    plt.close()
...
def main():
    ...
    parser.add_argument("--t1_features", required=True)
    parser.add_argument("--out_dir",     required=True)
    parser.add_argument("--figs_dir",    default="figs")
    parser.add_argument("--top_k",            type=int,   default=20)
    parser.add_argument("--penalize_regex",   default="nboundary")
    ...
    selected_slopes = select_slope_features_train_only(
        X_slopes_all.loc[idx_train], args.top_k, args.penalize_regex,
        args.penalty_factor, args.winsor_low, args.winsor_high,
    )

    t1_medians   = df[t1_cols].loc[idx_train].median()
    X_t1_all     = df[t1_cols].fillna(t1_medians)
    all_features = selected_slopes + t1_cols
    X_all        = pd.concat([X_slopes_all[selected_slopes], X_t1_all], axis=1)

    print(f"\n{'='*80}")
    print(f"COMBINED: {len(selected_slopes)} BSC slopes + {len(t1_cols)} T1 = {len(all_features)} total")
    print(f"{'='*80}")

    X_train_raw = X_all.loc[idx_train].copy()
    X_test_raw  = X_all.loc[idx_test].copy()

    X_tr_log = signed_log1p_df(X_train_raw)
    X_te_log = signed_log1p_df(X_test_raw)
    limits   = fit_winsor_limits(X_tr_log, args.winsor_low, args.winsor_high)
    X_tr_r   = apply_winsor_limits(X_tr_log, limits)
    X_te_r   = apply_winsor_limits(X_te_log, limits)
    X_train_scaled, X_test_scaled, _ = minmax_scale_train_test(X_tr_r, X_te_r)

    model, metrics, pred_train, pred_test = train_rsf_model(
        X_train_scaled, X_test_scaled, y_train, y_test, args.n_estimators
    )

    with open(out_dir / "rsf_combined_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)
    with open(out_dir / "rsf_combined_features.txt", "w") as f:
        f.write("\n".join(all_features))
    pd.DataFrame({
    ...
    }).to_csv(out_dir / "rsf_combined_predictions.csv", index=False)
    pd.DataFrame({
        "subject":        df.loc[idx_train, "subject"].values,
        "true_time":      y_train["time_years"].values,
        "event":          y_train["event"].values,
        "predicted_risk": pred_train,
    }).to_csv(out_dir / "rsf_combined_train_predictions.csv", index=False)
    ...
    cv_results = run_5fold_cv(
        df, slope_cols, t1_cols, y_all, args.n_estimators,
    ...
    with open(out_dir / "rsf_combined_cv.json", "w") as f:
    ...
    print(f"  5-fold CV test     : {np.mean(cv_results['test']):.4f} ± {np.std(cv_results['test']):.4f}")
    print(f"  Overfitting (70/30): {metrics['overfitting_gap']:.4f}")
    print(f"{'='*80}\n")
